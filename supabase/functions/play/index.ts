...
const ERROR_STATUS: Record<string, number> = {
  ...
  // Multiplayer. Unmapped codes fall through to 400, which is how a refusal
  // the server understood perfectly turns into an unexplained "bad request" in
  // someone's network tab — so every code the mp_* RPCs can return is listed.
  //
  // `table_unavailable` is deliberately one code for four situations: bad
  // code, already full, expired, your own table. A guesser who could tell them
  // apart could enumerate live invite codes.
