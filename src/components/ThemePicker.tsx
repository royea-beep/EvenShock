 ...
 * `data-theme`, so the swatch inside renders the REAL choice buttons under that
 * theme's tokens — the pattern only works because the Tailwind utilities are
 * built with `@theme inline` and resolve through the cascade at runtime.
 */
        ...
        {THEMES.map((option, index) => {
          ...
          return (
            <button
              ...
              className={`flex cursor-pointer flex-col items-center gap-2 p-3 transition-transform focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-current hover:scale-[1.03] ${
                selected ? 'scale-[1.03]' : 'opacity-85'
              }`}
            >
              {/* The actual game buttons, rendered under this theme's tokens. */}
              <span className="flex items-center gap-1.5">
                {PREVIEW_CHOICES.map((choice) => (
                  <ChoiceButton key={choice} choice={choice} preview />
                ))}
              </span>

              <span className="display-type theme-preview-label text-[0.68rem] font-bold text-ink">
                {option.name}
              </span>

              {/* Selection is announced by aria-checked and marked with a glyph,
                  so it never relies on the highlight color alone. */}
