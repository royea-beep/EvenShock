...
export function ChoiceButton({ choice, imageSet, onSelect, disabled }: ChoiceButtonProps) {
  const handleClick = () => {
    if (disabled) return;
    play('select');
    onSelect?.(choice);
  ...
  const size = 'h-[24vw] w-[24vw] sm:h-36 sm:w-36';
  const iconSize = 'h-[12vw] w-[12vw] sm:h-18 sm:w-18';
